...
async function handler(m) {
    ...
    if (!clanName) {
        return m.reply(
            `⚔️ *CREATE CLAN*\n\n` +
            `¡Crea un clan y recluta miembros!\n\n` +
            `Costo: *Rp ${CLAN_CREATE_COST.toLocaleString('id-ID')}*\n` +
            `Nombre máx.: *${MAX_CLAN_NAME} caracteres*\n\n` +
            `Ejemplo: *.clancreate DragonSlayer*`
        )
    }

    if (clanName.length > MAX_CLAN_NAME) {
        return m.reply(`❌ El nombre del clan máximo es de ${MAX_CLAN_NAME} caracteres`)
    }
    ...
    db.updateBerry(m.sender, -CLAN_CREATE_COST)
