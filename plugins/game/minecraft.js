...
import {
  ...
  healPlayer,
  JACKPOT_POOLS,
} from "../../src/lib/luffy-minecraft.js";
import {
  biomes,
  travelRequirements,
  pickaxes,
  ...
  SMELT_RECIPES,
  CRAFT_RECIPES,
} from "../../src/lib/luffy-minecraft-data.js";
...
let thumbMC = null;
try {
  const p = path.join(process.cwd(), "assets", "images", "luffy-minecraft.jpg");
...
function send(sock, m, text, title, body) {
  const msgId = sock.sendPreview(
    m.chat,
    {
      ...
      description: `⛏️ mina, 🛠️ fabrica, y ⚔️ pelea contra mobs en el mundo de minecraft`,
...
async function handler(m, { sock }) {
  ...
  if (cmd === "minecraft") {
    if (!sub || sub === "on" || sub === "off") {
      if (!m.isGroup) return m.reply("_Solo se puede alternar en grupos_");
  ...
  if (sub === "mine") {
    ...
    if (mc.miningPending && mc.miningPending.length > 0)
      return m.reply(`_📦 ¡Aún hay resultados de minería!_ \`.mct collect\` _primero._`);
    const now = Date.now();
    ...
    if (!pick) return m.reply(`_🪓 ¡No hay pico activo!_ \`.mct picks\``);
    ...
    await send(
      sock,
      m,
      `*_⛏️ Minando en ${biomes[bk]?.name || bk}..._*\n_🪓 Pico: ${pick.name} | 🍀 Luck: ${(st.luck * 100).toFixed(1)}%_`,
      "⛏️ Minando...",
      biomes[bk]?.name || "",
    );
    await new Promise((r) =>
    ...
    db.markDirty("users");
    return send(sock, m, txt, "⛏️ ¡Resultado de la Minería!", `💰 ${formatMoney(tv)}`);
  }
  ...
  if (sub === "sell") {
    ...
    if (!mc.inventory || mc.inventory.length === 0)
      return m.reply(`_🎒 ¡Inventario vacío!_ \`.mct mine\` _primero._`);
    let tv = 0;
  ...
  if (sub === "stats") {
    ...
    return send(sock, m, txt, "📊 Stats Detallados", pick ? pick.name : "");
  }
  ...
  if (sub === "orebook") {
    ...
    if (found.length === 0)
      return m.reply(`_📚 ¡Libro de Menas vacío!_ \`.mct mine\` _primero_`);
    let txt = `*📚 LIBRO DE MENAS* _(${found.length} menas)_\n\n`;
    ...
    return send(sock, m, txt.trim(), "📚 Libro de Menas", `🧱 ${found.length} menas`);
  }
  ...
  if (sub === "travel") {
    ...
    if (!biomes[tk]) return m.reply(`_🗺️ ¡El biome no existe!_ \`.mct travel\``);
    if (mc.currentBiome === tk)
      return m.reply(`_📍 ¡Ya estás en ${biomes[tk].name}!_`);
    ...
    return send(
      sock,
      m,
      `*🚪 ¡CAMBIO DE BIOME!*\n\n📍 Ahora estás en *${biomes[tk].name}*\n🧱 ${biomes[tk].listOre.length} tipos de menas disponibles`,
      "🚪 ¡Viaje!",
      biomes[tk].name,
    );
  }
  ...
  if (sub === "buy") {
    ...
    if (pickaxes[pk].price === 0)
      return m.reply(`_🪙 ¡Este pico es de Token/Prestigio!_`);
    ...
    return send(
      sock,
      m,
      `*🛍️ ¡PICO COMPRADO!*\n\n*${pickaxes[pk].name}*\n_🪓 Escribe_ \`.mct equip ${pk}\` _para equiparlo_`,
      "🪓 ¡Pico Nuevo!",
      pickaxes[pk].name,
    );
  }
  ...
  if (sub === "equip") {
    ...
    return send(
      sock,
      m,
      `*🪓 ¡PICO EQUIPADO!*\n\n*${mc.pickaxes[pk].name}* _✅ ahora está activo_`,
      "🪓 ¡Equipar Pico!",
      mc.pickaxes[pk].name,
    );
  }
  ...
  if (sub === "enchant") {
    ...
    const pk = mc.usedPickaxe || "woodpick";
    const pick = mc.pickaxes[pk];
    if (!pick) return m.reply(`_🪓 ¡No hay pico activo!_`);
  ...
  if (sub === "pickup") {
    ...
    if (pick.level >= pick.maxLevel)
      return m.reply(`_⬆️ ¡El pico ya está al nivel máximo!_`);
    const cost =
  ...
  if (sub === "fight") {
    ...
    if (!mobData[mk]) return m.reply(`_👹 ¡El mob no existe!_ \`.mct fight\``);
  ...
  if (sub === "inv" || sub === "inventory") {
    ...
    const mc = user.minecraft;
    if (!mc.inventory || mc.inventory.length === 0)
      return m.reply(`_🎒 ¡Inventario vacío!_`);
    const grouped = {};
    ...
    switch (item.type) {
      case "pickaxe":
        if (pickaxes[item.value] && !mc.pickaxes[item.value]) {
          ...
          txt += `🪓 ¡CONSEGUISTE UN PICO: *${item.label}*\n`;
      ...
      case "enchant_scroll": {
        ...
        if (avail.length > 0) {
          ...
          if (mc.pickaxes[pk2]) {
            ...
            txt += `✨ Encantamiento: *${ed.name}* _(${item.value})_ en el pico!\n`;
          }
        }
        break;
      }
  ...
  if (sub === "upgrade") {
    ...
    if (!type || !UPGRADES[type]) {
      ...
      for (const [k, u] of Object.entries(UPGRADES)) {
        ...
        txt += `*${u.name}* _(Lv.${lv}/${u.maxLevel})_\n  _${u.desc}_\n  ${lv >= u.maxLevel ? "_✅ MÁXIMO_" : `_💰 Siguiente: ${formatMoney(u.getCost(lv))}_`}\n\n`;
      }
      return send(
        sock,
        m,
  ...
  if (sub === "prestige") {
    ...
    const allReqs = [
      { lv: 1, blocks: 500, money: 2e10, rw: "Pico de Prestigio + 60 tokens" },
      { lv: 2, blocks: 1500, money: 2e12, rw: "Luck +20% + 180 tokens" },
      { lv: 3, blocks: 4000, money: 2e14, rw: "Pico Cósmico + 600 tokens" },
      { lv: 4, blocks: 10000, money: 2e19, rw: "2x EXP + 1200 tokens" },
      { lv: 5, blocks: 25000, money: 2e22, rw: "Pico Omega + 6000 tokens" },
    ];
    for (const r of allReqs)
      txt += `${cp >= r.lv ? "✅" : "🔒"} *P${r.lv}*: _🧱 ${r.blocks} bloques | 💰 ${formatMoney(r.money)}_\n  _${r.rw}_\n\n`;
    return send(
      sock,
      m,
  ...
  if (sub === "jackpot") {
    ...
    const mc = user.minecraft;
    if (!sa[0]) {
      let txt = `*🎰 SISTEMA DE JACKPOT*\n\n_💎 Sistema jackpot con premios super grandes!_\n_🎁 Puede dar _Premium_, _Partner_, _Energía_, _Limit_, incluso _UNLIMITED_!_\n\n`;
