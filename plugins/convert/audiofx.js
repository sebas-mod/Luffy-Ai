...
const EFFECTS = {
    ...
    deep:      { emoji: '🎤', filter: 'asetrate=44100*0.7,atempo=1.3', desc: 'Voz profunda' },
    earrape:   { emoji: '📢', filter: 'volume=10,bass=g=30:f=80:w=0.6,acrusher=level_in=8:level_out=12:bits=4:mode=log:aa=1', desc: 'Earrape' },
    echo:      { emoji: '🔁', filter: 'aecho=0.8:0.88:60:0.4', desc: 'Eco/resonancia' },
    ...
    superslow: { emoji: '🦥', filter: 'atempo=0.5', desc: 'Desacelera 2x' },
    tremolo:   { emoji: '〰️', filter: 'tremolo=f=8:d=0.7', desc: 'Trémolo / vibración' },
    vibrato:   { emoji: '🎸', filter: 'vibrato=f=7:d=0.5', desc: 'Vibrato' },
    phone:     { emoji: '📞', filter: 'highpass=f=300,lowpass=f=3400,volume=1.5', desc: 'Voz de teléfono' },
...
function buildEffectList() {
    ...
    txt += `_Ejemplo: responde un audio y luego escribe .bass_`
    return txt
}
...
async function handler(m, { sock }) {
    ...
    try {
        ...
        if (!buffer?.length) {
            return m.reply(`❌ No se pudo descargar el medio`)
        }
