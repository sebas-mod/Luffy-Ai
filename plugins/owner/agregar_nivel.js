...
const pluginConfig = {
    name: 'agregar_nivel',
    alias: ["givelevel", "addlvl"],
    category: 'owner',
    description: 'Añadir nivel a un usuario (vía exp)',
    usage: '.agregar_nivel <cantidad> @user',
    example: '.agregar_nivel 5 @user',
    isOwner: true,
    isPremium: false,
    isGroup: false,
    isPrivate: false,
...
async function handler(m, { sock }) {
    ...
    if (!targetJid || levels <= 0) {
        return m.reply(
            ...
            `*EJEMPLO DE USO:*\n` +
            `- *${m.prefix}agregar_nivel 5*\n` +
            `- *${m.prefix}agregar_nivel 10 @user*`
        )
    }
