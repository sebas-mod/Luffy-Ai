...
const pluginConfig = {
  ...
  usage: ".estado_grupo_todo <texto> o responde a un medio",
  example: ".estado_grupo_todo ¡Anuncio importante!",
  isOwner: true,
  isPremium: false,
  isGroup: false,
  isPrivate: false,
...
async function handler(m, { sock, db }) {
  ...
  if (source) {
    ...
    return m.reply(
      `⚠️ *ᴄᴏ́ᴍᴏ ᴜsᴀʀʟᴏ*\n\n` +
        `> \`${m.prefix}estado_grupo_todo texto\` - Story de texto a todos los grupos\n` +
        `> Responde a imagen/video/audio + \`${m.prefix}estado_grupo_todo\`\n` +
        `> Envía imagen/video + caption \`${m.prefix}estado_grupo_todo\`\n\n` +
      ...
      interactiveButtons: [
        {
          ...
          buttonParamsJson: JSON.stringify({
            ...
            id: `${m.prefix}estado_grupo_todo --yes`,
          }),
        },
        {
          name: "quick_reply",
