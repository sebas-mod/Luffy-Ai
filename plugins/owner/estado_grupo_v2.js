...
const pluginConfig = {
  name: "estado_grupo_v2",
  alias: ["statusgrupv2"],
  category: "owner",
  description: "Publicar Group Status V2 en el grupo elegido",
  usage: ".estado_grupo_v2 <texto> o responde a un medio",
  example: ".estado_grupo_v2 ¡Hola a todos!",
  isOwner: true,
  isPremium: false,
  isGroup: false,
  isPrivate: false,
...
async function handler(m, { sock, db }) {
  ...
  if (
    ...
    await m.reply(
      `⚠️ *ᴄóᴍᴏ ᴜsᴀʀ*\n\n` +
      `> \`${m.prefix}estado_grupo_v2 texto\` - Estado de texto\n` +
      `> Responde imagen/video/audio + \`${m.prefix}estado_grupo_v2\`\n` +
      `> Envía imagen/video con caption \`${m.prefix}estado_grupo_v2\``,
    );
    return;
  }
  ...
  try {
    ...
    const groupRows = groupList.map(([id, meta]) => ({
      ...
      description: id,
      id: `${m.prefix}estado_grupo_v2 --confirm ${id}`,
    }));
