...
const pluginConfig = {
    name: ['crear_grupo', 'creategroup', 'newgroup'],
    alias: [],
    category: 'owner',
    description: 'Crear un grupo nuevo',
    usage: '.crear_grupo <nombre>|<numero1,numero2,...>|<duracion_minutos>',
    example: '.crear_grupo Grupo Nuevo|628xxx,628yyy|60',
    isOwner: true,
    cooldown: 5,
    carne: 0,
    isEnabled: true
...
async function handler(m, { sock }) {
    ...
    if (args.length < 2) {
        ...
        txt += `*Cómo Usar:*\n`
        txt += `👉 \`${m.prefix}crear_grupo Nombre del Grupo | 628xxx,628yyy | Duración(minutos)\`\n\n`
        txt += `*Detalles:*\n`
        ...
        txt += `\`${m.prefix}crear_grupo Equipo Alfa | 628123,628456\`\n\n`
        txt += `*Ejemplo Con Duración (Periodo Activo 60 Minutos):*\n`
        txt += `\`${m.prefix}crear_grupo Equipo Beta | 628123,628456 | 60\``
        return m.reply(txt)
    }
