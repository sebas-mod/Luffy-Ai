...
async function handler(m, { sock }) {
  ...
  try {
    ...
    await new Promise((resolve, reject) => {
      ...
      output.on("close", () => {
        try {
          if (!fs.existsSync(zipFilePath)) {
            fail(new Error("El archivo de respaldo no se encontró después del proceso zip"));
            return;
          }
          const stats = fs.statSync(zipFilePath);
          if (stats.size <= 0) {
            fail(new Error("El archivo de respaldo está vacío o es 0KB"));
            return;
          }
          succeed();
        } catch (error) {
      ...
      if (fileCount === 0) {
        fail(new Error("No hay archivos incluidos en el respaldo del script"));
        return;
      }
      archive.finalize();
    });
