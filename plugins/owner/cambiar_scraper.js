...
async function handler(m, { sock }) {
  ...
  if (args[0]?.toLowerCase() === "list") {
    ...
    text +=
      ...
      `> Usa \`${m.prefix}cambiar_scraper <nombre>\` con el código respondido`;
  ...
  if (!quoted) {
    return m.reply(
      `🔄 *CAMBIAR SCRAPER*\n\n` +
        ...
        `\`${m.prefix}cambiar_scraper\` - Detección automática desde el export\n` +
        `\`${m.prefix}cambiar_scraper nombrearchivo\` - Nombre de archivo personalizado\n\n` +
        `📋 *Ver la lista de scrapers:*\n` +
        `\`${m.prefix}cambiar_scraper list\`\n\n` +
