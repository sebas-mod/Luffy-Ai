...
const pluginConfig = {
    ...
    usage: '.editar_lista <numero> <campo> <valor>',
    example: '.editar_lista 1 isi Contenido nuevo aquí',
    isOwner: true,
    isPremium: false,
    isGroup: false,
    isPrivate: true,
...
async function handler(m, { sock }) {
    ...
    const match = text.match(/^(\d+)\s+(nombre|contenido|descripcion|imagen|video)\s*(.*)/i)
    ...
    if (!match) {
        return m.reply(
            ...
            `📌 *Campos editables:*\n` +
            `• *nombre* 🏷️ — Título de la información\n` +
            `• *contenido* 📝 — Contenido de la información (usa \`;;\` para nueva línea)\n` +
            `• *descripcion* 📋 — Descripción corta (vista previa en la lista)\n` +
            `• *imagen* 🖼️ — Subir nueva imagen (responde una imagen)\n` +
            `• *video* 🎬 — Subir nuevo video (responde un video)\n\n` +
            `📝 *Ejemplos:*\n` +
            `\`${m.prefix}editar_lista 1 contenido Términos nuevos: blablabla;;Condiciones: blablabla\`\n` +
    ...
    switch (field) {
        ...
        default:
            return m.reply(`❌ *Campo no reconocido.*\n\nUsa: nombre, contenido, descripcion, imagen, video 📋`)
    }
