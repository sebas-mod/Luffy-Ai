...
const pluginConfig = {
    name: 'calendario',
    alias: ['calendar', 'fechas'],
    category: 'canvas',
    description: 'Ver el calendario completo del año 2026 o por mes.',
    usage: '.calendario [mes/año]',
    example: '.calendario enero',
    isOwner: false,
    isPremium: false,
    isGroup: false,
    isPrivate: false,
...
const LIBUR = {
  "1-1":  { nombre: "Año Nuevo (Masehi)", tipo: "nasional" },
  "1-16": { nombre: "Isra Mi'raj del Profeta Muhammad SAW", tipo: "keagamaan" },
  "2-16": { nombre: "Día libre conjunto (Año Nuevo Chino)", tipo: "cuti" },
  "2-17": { nombre: "Año Nuevo Chino 2577 Kongzili", tipo: "budaya" },
  "3-18": { nombre: "Día libre conjunto (Nyepi)", tipo: "cuti" },
  "3-19": { nombre: "Nyepi (Año Nuevo Saka 1948)", tipo: "keagamaan" },
  "3-20": { nombre: "Día libre conjunto (Idul Fitri)", tipo: "cuti" },
  "3-21": { nombre: "Idul Fitri 1447 H", tipo: "keagamaan" },
  "3-22": { nombre: "Idul Fitri 1447 H", tipo: "keagamaan" },
  "3-23": { nombre: "Día libre conjunto (Idul Fitri)", tipo: "cuti" },
  "3-24": { nombre: "Día libre conjunto (Idul Fitri)", tipo: "cuti" },
  "4-3":  { nombre: "Muerte de Jesucristo (Viernes Santo)", tipo: "keagamaan" },
  "4-5":  { nombre: "Resurrección de Jesucristo (Pascua)", tipo: "keagamaan" },
  "5-1":  { nombre: "Día Internacional del Trabajo", tipo: "nasional" },
  "5-14": { nombre: "Ascensión de Jesucristo", tipo: "keagamaan" },
  "5-15": { nombre: "Día libre conjunto (Ascensión de Jesucristo)", tipo: "cuti" },
  "5-27": { nombre: "Idul Adha 1447 H", tipo: "keagamaan" },
  "5-28": { nombre: "Día libre conjunto (Idul Adha)", tipo: "cuti" },
  "5-31": { nombre: "Waisak 2570 BE", tipo: "keagamaan" },
  "6-1":  { nombre: "Día del nacimiento de Pancasila", tipo: "nasional" },
  "6-16": { nombre: "Año Nuevo Islámico 1448 H", tipo: "keagamaan" },
  "8-17": { nombre: "Día de la Independencia de la República de Indonesia", tipo: "nasional" },
  "8-25": { nombre: "Maulid del Profeta Muhammad SAW", tipo: "keagamaan" },
  "12-24": { nombre: "Día libre conjunto (Navidad)", tipo: "cuti" },
  "12-25": { nombre: "Día de Navidad", tipo: "keagamaan" },
};
...
async function pastikanFontTersedia() {
  ...
  for (const f of daftarFont) {
    ...
    try {
      ...
      console.warn(`No se pudo descargar la fuente ${f.file} (${e.message}), usando la fuente predeterminada del sistema.`);
    }
  }
}
...
function gambarHeader(ctx, x, y, w, opts) {
  ...
  ctx.save();
  const eyebrow = "AÑO 1948 SAKA  \u00b7  1447\u20131448 HIJRI";
...
function gambarLegenda(ctx, x, y, w, h) {
  ...
  const item = [
    { label: "Día Nacional", warna: WARNA.tipo.nasional },
    { label: "Fiestas Religiosas", warna: WARNA.tipo.keagamaan },
    { label: "Día Cultural", warna: WARNA.tipo.budaya },
    { label: "Día Libre Conjunto", warna: WARNA.tipo.cuti },
  ];
...
function gambarKartuBulan(ctx, x, y, w, tinggiKartu, bulanNum, namaBulan, skala) {
  ...
  for (let i = 0; i < totalHari; i++) {
    ...
    if (info) {
      ...
      ctx.fillStyle = WARNA.tipo[info.tipo];
      ctx.fill();
      ctx.fillStyle = WARNA.tipoTexto[info.tipo];
  ...
  if (libur.length === 0) {
    ...
    libur.forEach((l) => {
      ...
      ctx.fillText(l.nombre, x + 44 * s, yDaftar);
      yDaftar += 19 * s;
    });
  }
}
...
async function renderTahun(outputPath) {
  ...
  gambarHeader(ctx, 0, 0, lebarKanvas, {
    ...
    subjudulBaris: [
      "Doce meses completos con días festivos nacionales, fiestas",
      "religiosas, celebraciones culturales y días libres conjuntos.",
    ],
    statistik: [
      { angka: "17", label: "Festivos Nacionales" },
      { angka: "8", label: "Días Libres Conjuntos" },
      { angka: "25", label: "Total de Días Festivos" },
    ],
  });
...
async function renderBulan(bulanNum, outputPath) {
  ...
  gambarHeader(ctx, 0, 0, lebarKanvas, {
    ...
    subjudulBaris: [
      "Calendario de Indonesia con días festivos nacionales, fiestas",
      "religiosas, celebraciones culturales y días libres conjuntos.",
    ],
    statistik: [{ angka: String(libur.length), label: "Días Festivos Este Mes" }],
  });
...
async function handler(m, { sock, text }) {
    try {
        ...
        if (text) {
            ...
            if (NAMA_KE_NOMOR[input]) {
                ...
                return m.reply(
                    ...
                    `Por favor, escribe el nombre de un mes correcto (ejemplo: *enero*, *febrero*, etc.) o déjalo vacío para ver el calendario completo del año.`
                );
            }
        }
        ...
        const outputPath = path.join(tempDir, `calendario_${mode}_${Date.now()}.png`);
