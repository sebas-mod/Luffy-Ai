...
const pluginConfig = {
    ...
    usage: '.crear_citas [id fondo] | <texto> | [nombre del creador]',
    example: '.crear_citas 2 | Sigue viviendo aunque seas inútil | Maman',
    isOwner: false,
    isPremium: false,
    isGroup: false,
    isPrivate: false,
...
async function handler(m, { sock, text }) {
    if (!text) {
        return m.reply(
            ...
            `*CÓMO USARLO:*\n` +
            `- \`${m.prefix}crear_citas <texto>\`\n` +
            `- \`${m.prefix}crear_citas <id_fondo> | <texto>\`\n` +
            `- \`${m.prefix}crear_citas <id_fondo> | <texto> | <tu nombre>\`\n\n` +
            ...
            `- 7: Light Yagami\n` +
            `- 8: Higuruma\n\n` +
            `_Ejemplo: ${m.prefix}crear_citas 2 | Sigue viviendo aunque seas inútil | Maman_`
        );
    }
