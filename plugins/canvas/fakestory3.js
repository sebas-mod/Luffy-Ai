...
const pluginConfig = {
  ...
  category: "canvas",
  description: "Crea una historia ficticia de Instagram con texto superpuesto.",
  usage: ".fakestory3 <nombre>|<texto1>|<texto2>",
  example: ".fakestory3 Luffy|¡Seré el Rey!|¡de los Piratas!",
  isOwner: false,
  isPremium: false,
  isGroup: false,
  isPrivate: false,
...
async function handler(m, { sock }) {
  ...
  if (!input || !input.includes("|")) {
    return m.reply(
      `📷 *ꜰᴀᴋᴇ sᴛᴏʀʏ 3*\n\n` +
        `> Responde a una imagen con este formato:\n` +
        `> \`${m.prefix}fakestory3 nombre|texto1|texto2\`\n\n` +
        `> Ejemplo:\n` +
        `> \`${m.prefix}fakestory3 Luffy|¡Seré el Rey!|¡de los Piratas!\``,
    );
  }
  ...
  try {
    ...
    if (!isImage) {
      m.react("❌");
      return m.reply(`❌ *ERROR*\n\n> ¡Responde a una imagen para crear la historia ficticia!`);
    }
    let imageBuffer;
