...
async function handler(m, { sock }) {
  ...
  try {
    ...
    if (!data || !data.streams_array || data.streams_array.length === 0) {
      await m.react("❌");
      return m.reply("⚠️ Error al extraer el video. Asegúrate de que el enlace de Shopee sea correcto y público.");
    }

    const best = bestStream(data.streams_array);
    const videoUrl = best.stream_url;

    let caption = `🛍️ *SHOPEE VIDEO DESCARGADOR* 🛍️\n\n`;
    ...
    await m.react("☢");
    m.reply("😔 Error al descargar video de Shopee.");
  }
}
