...
async function handler(m, { sock }) {
  ...
  if (!url) {
    return m.reply(
      `📌 *PINTEREST DESCARGADOR*\n\n` +
        `> Descarga imagen/video de Pinterest\n\n` +
        `*EJEMPLO:*\n` +
  ...
  if (!url.includes("pinterest") && !url.includes("pin.it")) {
    return m.reply("❌ URL no válido. Usa un enlace de Pinterest.");
  }
  m.react("🕕");
  try {
    const res = await axios.get(`https://api.azbry.com/api/download/pinterest?url=${encodeURIComponent(url)}`);
    if (!res.data || !res.data.status || !res.data.result) {
      throw new Error("Error al obtener datos de la API de Pinterest.");
    }
    ...
    if (mediaList.length === 0) {
      throw new Error("No se encontró ningún medio");
    }
    ...
    for (const media of mediaList) {
      if (media.type === "video") {
        ...
        try {
            ...
            try {
                // Fallback: Pinterest bloquea axios genérico (403), así que se descarga manual con User-Agent
        ...
        if (media.url.includes("gif")) {
          ...
          try {
            ...
            if (!raw) throw new Error("Error al descargar GIF");
            ...
            if (!fs.existsSync(mp4Path)) throw new Error("Error al convertir GIF");
