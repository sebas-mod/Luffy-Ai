...
async function handler(m, { sock }) {
  ...
  if (!url || !/threads/i.test(url)) {
    return m.reply("❌ *¡Vaya, el enlace de Threads no está!*\n\nDebes ingresar un enlace (link) de la publicación de Threads que quieres descargar. ¡Asegúrate de que el enlace sea correcto! \n\nEjemplo: `.tdl https://www.threads.net/@zuck/post/xxx`");
  }
  ...
  try {
    ...
    if (res.status >= 300 || data.success !== true || result.length === 0) {
      await m.react("❌");
      return m.reply(`⚠️ *¡Ay, hubo un error al obtener datos de Threads!*\n\nPuede que esta publicación sea privada, haya sido eliminada, o el enlace que proporcionaste no sea correcto.\n\nRazón del sistema: ${data.message || data.error || "Desconocido"}`);
    }

    const captionText = `✨ *THREADS DESCARGADOR* ✨

¡Hola! Aquí tienes la descarga de Threads que pediste:
...
*¡Espero que te sea útil!* No olvides volver si quieres descargar algo más. 🚀`;
    ...
    await m.react("☢");
    m.reply("😔 *Parece que hay un problema con el sistema.* \n\nOcurrió un error al intentar procesar el enlace de Threads. ¡Por favor intenta de nuevo más tarde!");
  }
}
