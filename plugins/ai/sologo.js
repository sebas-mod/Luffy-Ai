...
const pluginConfig = {
  ...
  category: "ai",
  description: "Crear logos usando IA a partir de texto (prompt)",
  usage: ".sologo <prompt>",
  example: ".sologo Gato lindo color azul",
  isOwner: false,
  isPremium: false,
  isGroup: false,
  isPrivate: false,
...
async function handler(m, { sock }) {
  ...
  if (!prompt) {
    return m.reply("❌ Ingresa la descripción del logo que deseas crear.\n\nEjemplo: `.sologo robot genial color rojo`");
  }
  ...
  try {
    ...
    if (!data.status || !data.result || data.result.length === 0) {
      await m.react("❌");
      return m.reply("⚠️ La IA falló al crear el logo. Intenta usar una descripción diferente.");
    }
    ...
    await m.react("☢");
    m.reply("😔 Ocurrió un error al procesar la solicitud de IA.");
  }
}
