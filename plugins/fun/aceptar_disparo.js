...
const pluginConfig = {
  name: "aceptar_disparo",
  alias: ["accept", "yes"],
  category: "fun",
  description: "Acepta el disparo de alguien",
  usage: ".aceptar_disparo @tag",
  example: ".aceptar_disparo @628xxx",
  isOwner: false,
  isPremium: false,
  isGroup: true,
  isPrivate: false,
...
async function handler(m, { sock }) {
  ...
  if (!shooterJid) {
    return m.reply(
      `⚠️ *ᴄᴏ́ᴍᴏ ᴜsᴀʀ*\n\n` +
        `> Responde al mensaje de disparo + \`${m.prefix}aceptar_disparo\`\n` +
        `> O \`${m.prefix}aceptar_disparo @tag\``,
    );
  }
