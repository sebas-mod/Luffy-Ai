...
const pluginConfig = {
  ...
  alias: ["growagarden2"],
  category: "info",
  description: "Revisa el stock de Grow a Garden con la función watch",
...
function formatStock(d) {
  ...
  lines.push("");
  lines.push(`- ⏰ Reabastecimiento en : *${d.restockInLabel}*`);
  lines.push(`- 🔄 Rotación   : *${d.rotationId} (ciclo ${d.cycleSeconds}s)*`);
  lines.push(`- 📊 Estado     : *${d.status} (${d.staleness})*`);
  lines.push(`- 💾 Caché      : *${d.cache.servedFromCache ? "HIT" : "MISS"} (edad ${d.cache.ageSec}s/${d.cache.ttlSec}s)*`);
  lines.push("");
  if (d.weather?.active) {
    lines.push(`- ⛅ *CLIMA: ${d.weather.type.toUpperCase()}*`);
...
async function modeWatch(watchItems, m, sock) {
  ...
  await m.reply(`👀 *Vigilando:* ${watchItems.join(", ")}\n> Máx. 5 iteraciones (~2.5 min). Vuelve a ejecutarlo para ampliar.`);
  ...
  for (let i = 1; i <= 5; i++) {
    ...
    if (hits.length > 0) {
      let msg = `🎯 *[iter ${i}] ENCONTRADOS ${hits.length} elemento(s) vigilado(s)!*\n\n`;
      for (const h of hits) {
  ...
  await sock.sendMessage(m.chat, { text: `⏱️ 5 iteraciones hechas, sin coincidencias. Vuelve a ejecutarlo para continuar.` }, { quoted: m });
}
...
async function handler(m, { sock, args }) {
  ...
  try {
    ...
    if (isWatch) {
      ...
      if (items.length === 0) {
        return m.reply("❌ Introduce el nombre del item que quieres vigilar. Ejemplo: `.gag2 watch seed apple`");
      }
      m.react("✅");
