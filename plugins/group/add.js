...
const pluginConfig = {
    ...
    category: 'group',
    description: 'Agregar miembros al grupo (soporta varios)',
    usage: '.add <número1> [número2] [número3]... [link_grupo]',
...
async function handler(m, { sock }) {
    ...
    if (args.length === 0) {
        return m.reply(
            ...
            `> Requisitos:\n` +
            `> - El bot debe ser admin del grupo objetivo\n` +
    ...
    try {
        ...
        let resultText = `🥗 @${m.sender.split('@')[0]} agregó miembros al grupo\n\n`
        
        if (successList.length > 0) {
            resultText += `Hay *${successList.length}* miembros agregados correctamente:\n`
            successList.forEach(n => resultText += `• @${n}\n`)
            resultText += `\n`
        }
        
        if (invitedList.length > 0) {
            resultText += `📨 *Y también hay *${invitedList.length}* miembros invitados:*\n`
            invitedList.forEach(n => resultText += `• @${n}\n`)
            resultText += `\n`
        }
        
        if (failedList.length > 0) {
            resultText += `❌ *ᴇʀʀᴏʀ (${failedList.length}):*\n`
