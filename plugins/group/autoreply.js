...
const pluginConfig = {
    ...
    category: 'group',
    description: 'Configurar autoreply/smart triggers por grupo',
...
async function handler(m, { sock }) {
    ...
    if (action === 'global') {
        ...
        if (subAction === 'add') {
            ...
            return m.reply(
                ...
                `_Activo en todos los grupos y chat privado_`
            )
        }
        ...
        if (subAction === 'list' || !subAction) {
            ...
            text += `Activo en: *Todos los Grupos y Chat Privado*\n\n`
    ...
    if (!m.isGroup) {
        return m.reply(
            ...
            `_Nota: Para configurar autoreply de grupo, usa este comando dentro del grupo._`
        )
    }
    ...
    if (!action || action === 'status') {
        ...
        text += `Total Respuestas Personalizadas (Grupo): *${customReplies.length}*\n\n`
        ...
        text += `• *${m.prefix}autoreply reset* — Eliminar TODAS las personalizadas del grupo\n\n`
        ...
        if (m.isOwner) {
            ...
            text += `• *${m.prefix}autoreply private on/off* — Alternar respuesta del bot en DM\n\n`
        }
        
        text += `*CÓMO AÑADIR IMÁGEN:*\n`
        text += `1. Envía una imagen con caption: *${m.prefix}autoreply add trigger|reply*\n`
    ...
    if (action === 'on') {
        ...
        m.react('✅')
        return m.reply(`✅ *ᴀᴜᴛᴏʀᴇᴘʟʏ ᴅɪᴀᴋᴛɪꜰᴋᴀɴ*\n\n> El bot responderá automáticamente en este grupo ¡Shishishi!`)
    }
    ...
    if (action === 'add') {
        ...
        if (pipeIdx === -1) {
            return m.reply(
                ...
                `*Con imagen:*\n` +
                `1. Responde a imagen + ${m.prefix}ar add trigger|caption\n` +
    ...
    if (action === 'del' || action === 'rm' || action === 'remove') {
        ...
        return m.reply(
            ...
            `¡El trigger *${trigger}* fue eliminado con éxito!\n` +
            `Restante: *${customReplies.length}* respuestas`
        )
    }
    ...
    if (action === 'list') {
        ...
        let text = `📋 *LISTA AUTOREPLY DEL GRUPO*\n\n`
