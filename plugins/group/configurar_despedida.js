...
const pluginConfig = {
  name: "configurar_despedida",
  alias: ["customgoodbye"],
  category: "group",
  description: "Configurar un mensaje de despedida personalizado",
  usage: ".configurar_despedida <mensaje>",
  example: ".configurar_despedida Adiós {user}, nos vemos pronto!",
  isOwner: false,
  isPremium: false,
  isGroup: true,
  isPrivate: false,
...
async function handler(m, { sock }) {
  ...
  if (!text) {
    return m.reply(
      `📝 *ᴄᴏɴꜰɪɢᴜʀᴀʀ ᴅᴇsᴘᴇᴅɪᴅᴀ*\n\n` +
        ...
        `╰━ ⊱༺༒༻⊰ ━╯\n\n` +
        `\`Ejemplo:\`\n` +
        `\`${m.prefix}configurar_despedida Adiós {user}! 👋\`\n` +
  ...
  await m.reply(
    "୨୧〔 ❀ DESPEDIDA 〕୨୧\n"+`✅ Despedida configurada correctamente como *${text}*\n¿Quieres restablecerla? Escribe ${m.prefix}resetear_despedida`+"\n♡ ────── ♡",
  );
}
