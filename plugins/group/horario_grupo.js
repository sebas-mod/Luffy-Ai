...
const pluginConfig = {
    ...
    usage: '.horario_grupo <open/close> <HH:MM>',
    example: '.horario_grupo open 06:00',
    isOwner: false,
    isPremium: false,
    isGroup: true,
    isPrivate: false,
...
\`.jadwalgroup quitar open\`
\`.jadwalgroup quitar close\``;
        
        await m.reply(scheduleInfo);
        return;
    }
    
    if (action === 'quitar' || action === 'delete' || action === 'remove') {
        const type = args[1]?.toLowerCase();
        
        if (type !== 'open' && type !== 'close') {
            await m.reply(
                `⚠️ *ᴠᴀʟɪᴅᴀᴄɪóɴ ғᴀʟʟɪᴅᴀ*\n\n` +
                `> Uso: \`.jadwalgroup quitar open\`\n` +
                `> o: \`.jadwalgroup quitar close\``
            );
            return;
        }
