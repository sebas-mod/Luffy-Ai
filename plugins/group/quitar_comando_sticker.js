import { getQuotedStickerHash, deleteStickerCommand, listStickerCommands, findByCommand } from '../../src/lib/luffy-sticker-command.js'
const pluginConfig = {
    name: 'quitar_comando_sticker',
    alias: ['delcmdsticker', 'removesticker', 'unsticker'],
    category: 'group',
    description: 'Eliminar un sticker command',
    usage: '.quitar_comando_sticker <command> atau reply sticker',
    example: '.quitar_comando_sticker menu',
    isOwner: false,
    isPremium: false,
    isGroup: false,
    isPrivate: false,
    isAdmin: false,
    cooldown: 5,
    carne: 0,
    isEnabled: true
}

async function handler(m, { sock }) {
    const args = m.args || []
    const commandName = args[0]
    if (!commandName && !m.quoted) {
        const existingCmds = listStickerCommands()
        if (existingCmds.length === 0) {
            return m.reply(
                `🖼️ *sᴛɪᴄᴋᴇʀ ᴄᴏᴍᴍᴀɴᴅs*\n\n` +
                `> No hay sticker commands registrados.\n` +
                `> Agrégalos con \`.addcmdsticker\``
            )
        }
        
        let txt = `🖼️ *sᴛɪᴄᴋᴇʀ ᴄᴏᴍᴍᴀɴᴅs*\n\n`
        txt += `☽◯☾ ♰ 「 📋 *ʟɪsᴛᴀ* 」\n`
        
        for (const cmd of existingCmds) {
            txt += `┃ 🖼️ → \`.${cmd.command}\`\n`
        }
        txt += `╰━ ⊱༺༒༻⊰ ━╯\n\n`
        
        txt += `*Para eliminar:*\n`
        txt += `> \`.delstickercmd <command>\`\n`
        txt += `> o responde a un sticker + \`.delstickercmd\``
        
        return m.reply(txt)
    }
    
    let deleted = false
    let deletedCmd = ''
    if (m.quoted) {
        const stickerHash = getQuotedStickerHash(m)
        if (stickerHash) {
            const success = deleteStickerCommand(stickerHash)
            if (success) {
                deleted = true
                deletedCmd = 'el sticker al que respondiste'
            }
        }
    }
    if (!deleted && commandName) {
        const cleanCmd = commandName.toLowerCase().replace(/^\./, '')
        const found = findByCommand(cleanCmd)
        
        if (found) {
            const success = deleteStickerCommand(found.hash)
            if (success) {
                deleted = true
                deletedCmd = cleanCmd
            }
        } else {
            return m.reply(
                `❌ ¡El sticker command \`${cleanCmd}\` no fue encontrado!\n\n` +
                `> Ve la lista con \`.delstickercmd\``
            )
        }
    }
    
    if (deleted) {
        await m.react('✅')
        await m.reply(
            `✅ *sᴛɪᴄᴋᴇʀ ᴄᴏᴍᴍᴀɴᴅ ᴇʟɪᴍɪɴᴀᴅᴏ*\n\n` +
            `> 🗑️ \`${deletedCmd}\` fue eliminado.`
        )
    } else {
        await m.reply(
            `❌ ¡No se pudo eliminar!\n\n` +
            `> Responde al sticker que quieres eliminar, o\n` +
            `> Escribe el nombre del comando: \`.delstickercmd menu\``
        )
    }
}

export { pluginConfig as config, handler }