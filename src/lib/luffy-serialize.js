...
function createContextInfo(jid, text, title = "", body = "", thumbnail = null) {
  ...
  if (jid && text) {
    ...
    contextInfo.stanzaId = "Luffy-AiAI" + Date.now();
  }
    ...
    if (replyVariant === 2) {
      ...
      return sock.sendMessage(
        await ensureResolved(m.chat),
        {
          ...
          jpegThumbnail: srtImage ? await sharp(srtImage).resize(300, 300).toBuffer() : await sharp(getAssetBuffer("luffy2"))
            .resize(300, 300)
            .toBuffer(),
          caption: text,
          ...defaultOptions,
      ...
      const sss = getAssetBuffer("luffy3");
      return sock.sendMessage(
        await ensureResolved(m.chat),
        {
      ...
      const thumbnail = srtImage || getAssetBuffer("luffy");
      ...
      const thumbnailBuf = srtImage || getAssetBuffer("luffy");
      const fakeOrder = {
        key: {
          participant: "0@s.whatsapp.net",
          remoteJid: m.chat,
