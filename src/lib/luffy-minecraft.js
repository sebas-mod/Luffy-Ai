...
function addPickExp(mcUser, pickKey, amount) {
  ...
  if (levelUp)
    return `⛏️ Pico *${pick.name}* subió al nivel ${pick.level}!\n✨ ${statsIncreased.join(", ")}`;
  return null;
}
...
function doCombat(mcUser, mobKey) {
  ...
  if ((mcUser.level || 1) < mobDef.minLevel)
    return { error: `⬆️ ¡Nivel insuficiente! Necesitas Nv.${mobDef.minLevel}` };
...
function applyJackpotReward(db, mcUser, sender, result) {
  ...
  switch (type) {
    ...
    case "unlimited_carne": {
      ...
      applied = { desc: `♾️ UNLIMITED Carne!`, type };
      break;
    }
