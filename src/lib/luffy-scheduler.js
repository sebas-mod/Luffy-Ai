  ...
  const job = new CronJob(
    ...
    async () => {
      try {
        ...
        const resetCount = db.resetAllCarne(defaultLimit, -1);
        logger.success(
          "Scheduler",
  ...
  if (key === "dailylimitreset" || key === "limitreset" || key === "limit") {
    if (!activeCronJobs.has("dailyLimitReset")) {
      startDailyLimitReset({
        ...
        defaultLimit: cfg.carne?.default ?? 25,
      });
      started = true;
      schedulerName = "Daily Limit Reset";
    }
...
function initScheduler(config, sock = null) {
  if (config.features?.dailyLimitReset !== false) {
    startDailyLimitReset({
      ...
      defaultLimit: config.carne?.default ?? 25,
    });
  }
          ...
          if (group.scheduleOpen === currentTime) {
            try {
              ...
              if (
                ...
                try {
                  await groupScheduleSock.sendMessage(groupId, {
                    text: `⚠️ *ғᴀʟʟᴏ ᴇʟ ᴀᴜᴛᴏ ᴀʙʀɪʀ*\n\n> El bot no es admin, no puede cambiar los ajustes del grupo.\n> Haz al bot admin para activar esta función.`,
                  });
                } catch { }
              } else {
                logger.error(
      ...
      for (const [groupId, data] of Object.entries(sewaGroups)) {
        ...
        if (data.expiredAt <= now) {
          try {
            await sewaSock.sendText(
              groupId,
              `⏰ *ʀᴇɴᴛᴀ ᴇxᴘɪʀᴀᴅᴀ*\n\nEl período de renta del bot en este grupo terminó.\nEl bot dejará el grupo.\n\nContacta al capitán para renovar la renta.`,
              null,
              {
        ...
        if (remaining <= ONE_HOUR_MS && !data._warned1h) {
          try {
            ...
            await sewaSock.sendText(
              groupId,
              `⚠️ *ᴀᴠɪsᴏ ᴅᴇ ʀᴇɴᴛᴀ*\n\nQueda *${minutes} minutos* de renta!\nContacta al capitán para renovar.\n\nSi no se renueva, el bot saldrá automáticamente.`,
              null,
              {
                contextInfo: saluranCtx(),
              },
          ...
          try {
            ...
            await sewaSock.sendText(
              groupId,
              `⚠️ *ᴀᴠɪsᴏ ᴅᴇ ʀᴇɴᴛᴀ*\n\nQuedan *${days}d ${hours}h* de renta\nContacta al capitán para renovar.\n\nSi no se renueva, el bot saldrá automáticamente.`,
              null,
              {
                contextInfo: saluranCtx(),
              },
