...
import fs from "fs";
import { saluranCtx } from "./luffy-context.js";
import {
  delay,
  DisconnectReason,
  jidNormalizedUser,
...
import { addJadibotOwner } from "./luffy-jadibot-database.js";
import { extendSocket } from "./luffy-socket.js";
...
const ERROR_MESSAGES = {
  ...
  403: {
    ...
    action: "Este número pudo haber sido baneado por WhatsApp",
    fatal: true,
  },
  405: {
  ...
  406: {
    ...
    action: "Este número fue restringido por WhatsApp, espera unas horas",
    fatal: true,
  },
  408: {
  ...
  409: {
    ...
    action: "La sesión se está usando en otro dispositivo",
    fatal: true,
  },
  411: {
    reason: "Autenticación fallida",
    action: "Hay que escanear de nuevo el QR/pairing",
    fatal: true,
  },
  428: {
    reason: "Rate limit",
  ...
  440: {
    ...
    action: "Sesión expirada, hay que iniciar sesión de nuevo",
    fatal: true,
  },
  500: {
...
const CONNECTION_CLOSED_REASONS = [
  {
    ...
    action: "Posible número baneado o problema de red",
    fatal: true,
  },
  {
    match: /write EOF/i,
  ...
  },
  {
    ...
    action: "La cuenta cerró sesión desde el dispositivo",
    fatal: true,
  },
  {
    match: /replaced/i,
  ...
  },
  {
    ...
    action: "Hay que escanear de nuevo",
    fatal: true,
  },
  {
...
function parseDisconnectError(lastDisconnect) {
  ...
  return {
    ...
    action: "Contacta al admin si se repite",
    fatal: false,
    message: errorMessage,
  };
}
    ...
    if (connection === "close") {
      ...
      if (errorInfo.fatal || attempts >= MAX_RECONNECT_ATTEMPTS) {
        ...
        if (errorInfo.code === 403 || errorInfo.reason?.includes("banned")) {
          ...
          errorInfo.code === 406 ||
          errorInfo.reason?.includes("restring")
        ) {
          statusEmoji = "⚠️";
        }
        ...
        await safeSend(sock, m?.chat, {
          text:
            `${statusEmoji} *ᴊᴀᴅɪʙᴏᴛ ᴅᴇsᴄᴏɴᴇᴄᴛᴀᴅᴏ*\n\n` +
            `> 📱 Número: *@${id}*\n` +
            `> 🔢 Code: \`${errorInfo.code}\`\n` +
