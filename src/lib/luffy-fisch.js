...
const JACKPOT_POOLS = [
  {
    ...
    rewards: [
      ...
      { type: "carne", min: 5, max: 20, weight: 25 },
  ...
  },
  {
    ...
    rewards: [
      ...
      { type: "carne", min: 20, max: 100, weight: 20 },
  ...
  },
  {
    ...
    rewards: [
      ...
      { type: "carne", min: 100, max: 500, weight: 15 },
...
function applyJackpotReward(db, fischUser, sender, result) {
  ...
  switch (type) {
    ...
    case "carne": {
      ...
      const user = db.db.data.users[cleanJid];
      if (user) {
        user.carne = (user.carne || 0) + val;
        db.markDirty("users");
      }
      applied = { desc: `+${val} Carne`, type };
      break;
    }
    case "limit": {
      const cleanJid = sender.split("@")[0];
    ...
    case "unlimited_carne": {
      ...
      applied = { desc: `UNLIMITED Carne!`, type };
      break;
    }
