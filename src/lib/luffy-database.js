...
class Database {
  ...
  async init() {
    try {
      ...
      const currentDefault = config.carne?.default ?? 25;
      const currentPremium = config.carne?.premium ?? 100;
      const lastDefault = this.db.data.settings._lastCarneDefault;
      const lastPremium = this.db.data.settings._lastCarnePremium;
      ...
      if (lastDefault !== currentDefault || lastPremium !== currentPremium) {
        ...
        for (const jid in users) {
          const u = users[jid];
          if (u.carne === -1) continue;
          if (u.isPremium) {
            u.carne = currentPremium;
          } else {
            u.carne = currentDefault;
          }
          synced++;
        }
        this.db.data.settings._lastCarneDefault = currentDefault;
        this.db.data.settings._lastCarnePremium = currentPremium;
        this.markDirty("users");
        this.markDirty("settings");
        if (synced > 0) {
          logger.info(
            "database",
            `carne sync ${synced} user di-update (default: ${currentDefault}, premium: ${currentPremium})`,
          );
        }
      }
  ...
  async migrateFromSingleFile() {
    const oldFile = path.join(this.dbPath, "carnes.json");
    ...
    try {
      ...
      const backupPath = path.join(
        this.dbPath,
        `carnes.json.migrated.${Date.now()}.bak`,
      );
  ...
  setUser(jid, data = {}) {
    ...
    this.db.data.users[cleanJid] = {
      ...
      number: cleanJid,
      carne: data.carne ?? existing.carne ?? existing.energi ?? existingLimit,
      ...
      berry: data.berry ?? existing.berry ?? existing.koin ?? existingBalance,
  ...
  updateCarne(jid, amount) {
    const user = this.getUser(jid) || this.setUser(jid);
    if (!user) return 0;
    if (user.carne === -1) return -1;
    ...
    try {
      ...
      if (isOwnerUser && ownerCarne === -1) return -1;
      if (isPremiumUser && premiumCarne === -1) return -1;
    } catch { }

    user.carne = Math.max(0, (user.carne ?? 0) + amount);
    this.setUser(jid, user);
    return user.carne;
  }

  updateBerry(jid, amount) {
    const user = this.getUser(jid) || this.setUser(jid);
    if (!user) return 0;
    if (user.berry === -1) return -1;
    const MAX_BERRY = 9000000000000;
    user.berry = Math.max(0, Math.min(MAX_BERRY, (user.berry ?? 0) + amount));
    this.setUser(jid, user);
    return user.berry;
  }
  ...
  resetAllCarne(defaultCarne = 25, premiumCarne = -1) {
    let count = 0;
    for (const jid of Object.keys(this.db.data.users)) {
      const user = this.db.data.users[jid];
      user.carne = user.isPremium ? premiumCarne : defaultCarne;
      count++;
    }
    this.markDirty("users");
    return count;
