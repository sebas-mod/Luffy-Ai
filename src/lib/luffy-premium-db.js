...
function addOwner(jid, name = 'Unknown') {
    const cleanJid = jid?.replace(/@.+/g, '') || ''
    if (isOwner(cleanJid)) return { success: false, message: 'Ya es capitán' }
    ...
    saveOwners(owners)
    return { success: true, message: 'Agregado como capitán' }
}
...
function removeOwner(jid) {
    ...
    if (index === -1) return { success: false, message: 'No está en la lista de capitanes' }

    owners.splice(index, 1)
    saveOwners(owners)
    return { success: true, message: 'Eliminado de la lista de capitanes' }
}
...
function removePremium(jid) {
    ...
    if (index === -1) return { success: false, message: 'No está en la lista de premium' }

    premiumList.splice(index, 1)
    savePremium(premiumList)
    return { success: true, message: 'Eliminado del premium' }
}
...
function removePartner(jid) {
    ...
    if (index === -1) return { success: false, message: 'No está en la lista de partners' }

    partnerList.splice(index, 1)
    savePartners(partnerList)
    return { success: true, message: 'Eliminado de la lista de partners' }
}
