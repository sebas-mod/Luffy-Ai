...
function buildTextMessage(message, history = [], imageBuffer = null) {
  ...
  if (imageBuffer) {
    parts.push("El usuario envió una imagen. Si la imagen se puede analizar, usa ese contexto al responder.");
  }
...
async function chat({
  ...
  if (!apiKey) {
    throw new Error("La API key de logicBell no está configurada.");
  }
